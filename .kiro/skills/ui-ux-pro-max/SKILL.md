...
> **BetterMe project override (read first).** This is a reference/search tool, not the authority.
> This app is **Next.js 15 (App Router) + React 19 + Tailwind + Radix** — a web app, **NOT React
> Native**. The source of truth for design is the approved system in `docs/` + `src/app/globals.css`
> (semantic tokens) + the 4 themes (Cute Cat / Study Corner / Modern Focus / Minimal Calm). Never let
> a generic suggestion override the cozy Vietnamese identity or the no-guilt / no-decay invariants.
> Use this skill for inspiration — palettes, typography, UX checklists, chart ideas — then conform to
> the project's tokens and specs. On Windows run `python` (not `python3`).
...
- **Stack**: Next.js 15 (App Router) + React 19 + Tailwind + Radix — this project's web stack
...
```bash
python3 .kiro/skills/ui-ux-pro-max/scripts/search.py "atlantafx primer enterprise theme" --stack javafx
python3 .kiro/skills/ui-ux-pro-max/scripts/search.py "enterprise tableview density permission" --stack javafx
```
...
python3 .kiro/skills/ui-ux-pro-max/scripts/search.py "minimalism dark mode" --domain style

# Get UX best practices for search interaction and loading
python3 .kiro/skills/ui-ux-pro-max/scripts/search.py "search loading animation" --domain ux
```

### Step 4: Stack Guidelines

```bash
python3 .kiro/skills/ui-ux-pro-max/scripts/search.py "list performance navigation" --stack react-native
```
